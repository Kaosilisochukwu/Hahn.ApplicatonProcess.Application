...
constructor(controllerFactory:ValidationControllerFactory, http:HttpClient){
  this.http = http;this.controller = controllerFactory.createForCurrentScope();
  this.controller.addRenderer(new BootstrapFormRenderer());
  const res = fetch("http://localhost:5000/api/applicant/"+this.id)
  .then(res => res.json())
  .then(data => {
    ...
    this.familyName = data.familyName   
     ...
     this.country = data.countryOfOrigin
     this.hired = data.hired
  })
  ValidationRules.ensure((a: EditApplicants) => a.firstName)
  .required()
  .minLength(5)
  .ensure((a: EditApplicants) => a.familyName)
  .required()
  .minLength(5)
  .ensure((a: EditApplicants) => a.email)
  .required()
  .email()
  .ensure((a: EditApplicants) => a.address)
  .required()
  .minLength(10)
  .ensure((a: EditApplicants) => a.country)
  .required()
  .ensure((a: EditApplicants) => a.hired)
  .required()
  .ensure((a: EditApplicants) => a.age)
...
async addApplicant():Promise<any> {
  ...
  const applicantDetails:IApplicants = {
    id: this.id,
    name: this.firstName,
  ...
  const resp = await this.http.fetch("http://localhost:5000/api/applicant",{
    headers:{
      "Content-Type":"application/json"
    },
    method: "put",
...
getApplicants():any {
 ...
 this.http.fetch("http://localhost:5000/api/applicant",config).then(res=> res.json()).then(data=>{
  ...
  styleObject = "position: absolute; top: 0; background-color: rgba(0, 0, 0, 0.639); color: white; height: 100vh; width: 100%; display: flex; justify-content: center; align-items: center;"
}
